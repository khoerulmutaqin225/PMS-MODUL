...
class ShippingVessel(models.Model):
    ...
    active = fields.Boolean('Active', default=True)
    last_docking = fields.Date('Last Docking')
    intermediate_docking = fields.Date('Intermediate Docking')
    annual_docking = fields.Date('Annual Docking')
    status_doc_office = fields.Selection(
        string='Status Doc.Offhire',
        default='tidak',
        selection=[('ya', 'IYA'), ('tidak', 'TIDAK'), ])
...
class ShippingVesselCertificate(models.Model):
    ...
    status = fields.Char('Bulan Berjalan',size=32, compute="_compute_expired_status", store=True)
    remark = fields.Char('Remark',size=128)
    bisnis_unit = fields.Many2one('res.bisnis.unit',
                                  'Bisnis Unit',
                                  related="vessel_id.bisnis_unit",
                                  )
    dispensiasi_biaya = fields.Char('Dispensiasi Biaya', size=128)
    periode_compliance = fields.Char('Periode Compliance', size=128)
    status_compliance = fields.Char('Status Compliance', size=128)
    kategori_dokumen = fields.Char(
        string='Kategori Dokumen',
        related="type_id.status",
        readonly=True,)
    mgt_issue = fields.Selection(
        string='Mgt Issue',
        default='tidak',
        selection=[('tidak', 'TIDAK'), ('iya', 'IYA'), ])
    ...
    status_doc_office = fields.Selection(
        string='Status Doc.Offhire',
        related="vessel_id.status_doc_office",
        readonly=True,
        selection=[('ya', 'IYA'), ('tidak', 'TIDAK'), ])
